	...
	completeLayout: function completeLayout(newHeight, justify) {
		...
		this.items.forEach(function (item, i) {
			...
			if (self.rightToLeft) {
				...
				// Right-to-left.
				item.geometry.left = self.width - itemWidthSum - item.geometry.width;
			} else {

				// Left-to-right.
				item.geometry.left = itemWidthSum;
			}

			// Incrememnt width.
			itemWidthSum += item.geometry.width + self.spacing;
		});
		...
		if (justify) {
			...
			if (this.items.length === 1) {
				...
				singleItemGeometry = this.items[0].geometry;
				...
				this.items.forEach(function (item, i) {
					if (i > 0) {
						item.geometry.left -= roundedCumulativeErrors[i - 1];
						item.geometry.width -= roundedCumulativeErrors[i] - roundedCumulativeErrors[i - 1];
					} else {
						item.geometry.width -= roundedCumulativeErrors[i];
					}
				});
			}
		}
